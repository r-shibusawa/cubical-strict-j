...
## 2026-07-17 (32): 投稿準備完了 — マーカー除去・arXiv パッケージ・投稿手順書

(1) 論文からドラフト版マーカーを除去し最終コンパイル(38 頁・0 エラー・0 未定義参照)。(2) arXiv 投稿パッケージ `docs/paper/jrefl-arxiv.tar.gz`(jrefl.tex + lmcs.cls のみ)を作成。(3) `docs/paper/SUBMISSION.md` 新設: arXiv(アカウント作成〜cs.LO/math.LO、CC-BY 4.0、タイトル・プレーンテキスト abstract・Comments 欄の全文)と LMCS(episciences、arXiv ID 投稿方式、カバーレター全文)のコピー&ペースト用手順書。ユーザー操作 = arXiv アカウント作成(大学メール)→ tar.gz アップロード → announce 待ち(1–2 営業日)→ episciences で arXiv ID 投稿。main = 080823f。
