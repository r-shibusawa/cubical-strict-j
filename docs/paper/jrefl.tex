...
paper-to-probe correspondence (\texttt{ARTIFACT.md}) --- accompanies
the submission with a pinned toolchain and fixed commit; build with
\texttt{lake build}: every catalogue and switchover claim is an
elaboration-time guard, so a successful build is the verification
run.  We distinguish explicitly: the artifact provides
