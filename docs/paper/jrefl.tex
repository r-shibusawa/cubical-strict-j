...
\titlecomment{Draft v13, 2026-07-28.}
...
\begin{abstract}
  ...
  exists, closing what was previously left open.  Canonicity is
  accordingly stated as a conjecture: the natural computability
  argument breaks precisely at the conversion rule, and we exhibit
  the exact failing step.  The headline equation
  $\J\,d\,\refl \conv d$ is unaffected: it holds by a direct,
  self-contained evaluation argument on a $\Glue$-free, universe-free
  core fragment.  All concrete positive and negative
...
\item a fundamental theorem in \emph{conditional} form
  ...
  Theorem~\ref{thm:fundamental}) with termination folded into the
  logical relation; canonicity for unrestricted derivations is
  stated as a conjecture, the failing step of the previous argument
  exhibited exactly (Remark~\ref{rem:conversiongap}).  The
  base-rule cases are inherited as an explicitly delimited
...
\item \emph{Unconditional theorem} (Theorem~\ref{thm:core},
  ...
  (Proposition~\ref{prop:J}); canonicity is conjectural at every
  layer (Section~\ref{sec:nonsubst});
...
\noindent The status is honest and precise.  For derivations whose
conversion instances are environment-uniform, canonicity is a
theorem (the argument below, via
Theorem~\ref{thm:fundamental}).  For unrestricted
$T_{\mathrm{alg}}$-derivations it is open:
Remark~\ref{rem:conversiongap} shows the previous argument's
conversion case is unsound, and no repaired invariant is known to
us.  We conjecture canonicity holds --- evaluation is type-blind, so
conversions never block a closed computation, and every
machine-checked closed instance in the artifact evaluates to a
canonical form --- but a proof requires a new idea for typing-up-to
non-uniform conversion.  Consistency of the algorithmic equality
($0 \not\aconv 1$) and $\J\,d\,\refl \aconv d$ are \emph{not}
conditional: the former is a direct fact about the checker, the
latter is Proposition~\ref{prop:J}.
...
\begin{cor}[Minimal core, without base assumptions]\label{thm:core}
  ...
  the non-substitutivity counterexample lives in the core --- so
  unrestricted core canonicity is likewise
  Conjecture~\ref{thm:canonicity}.  What \emph{is} unconditional
  without any caveat is the headline equation:
  $\J\,d\,\refl \aconv d$, by the direct evaluation argument of
...
sound for $T_{\mathrm{eq}}$ with an exactly-specified checker;
canonicity, previously claimed, is re-scoped as a conjecture whose
failing proof step is exhibited exactly.  We regard the pair of
no-go theorems, and the delimitation of what survives them, as the
paper's contribution; no unconditional strict $\J$ for the full
univalent system is claimed.
...
equality.
(4)~\emph{Repairing canonicity}: an invariant for typing with
non-environment-uniform conversion instances
(Remark~\ref{rem:conversiongap}); the presentation question
previously listed here is settled negatively by
Corollary~\ref{cor:nopresentation}.
