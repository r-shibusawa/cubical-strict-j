...
% REMOVE BEFORE SUBMISSION: draft version marker.
\titlecomment{Draft v14, 2026-07-28.}
...
\begin{abstract}
  ...
  algorithmic equality a partial congruence --- an equivalence and a
  congruence, stable under weakening and renaming.  A \emph{second}
  no-go theorem shows this is sharp: the typed algorithmic equality
  is provably \emph{not} stable under general substitution --- a
  machine-checked equation between a transport redex and its
  structural unfolding leaves the equality under the substitution
  that makes the family constant --- and consequently \emph{no}
  substitution-closed presentation of the algorithmic equality
  exists, closing what was previously left open.  The binary
  (relational) fundamental theorem breaks precisely at the
  conversion rule, and we exhibit the exact failing step; canonicity
  is then \emph{regained} on a $\Glue$-free, universe-free core
  fragment by a new invariant --- unary, endpoint-blind hereditary
  termination, which depends only on type skeletons and therefore
  crosses the substitution barrier that kills the binary layer.  On
  that core, canonicity, termination and decidability of the
  algorithmic equality hold unrestrictedly, and the headline
  equation $\J\,d\,\refl \conv d$ holds outright; for the full
  univalent calculus with universes, canonicity remains
  conjectural.  All concrete positive and negative
...
\item a Kripke logical relation $\valeq$ over the domain, and the
  structural properties of the typed algorithmic equality it
  supports: an equivalence, a congruence for every term former,
  stable under weakening and renaming, coherent with the
  bidirectional checker (Theorem~\ref{thm:admissible});
\item a \emph{second no-go theorem}
  (Section~\ref{sec:nonsubst}, machine-checked): the typed
  algorithmic equality is provably \emph{not} stable under general
  substitution --- a transport redex and its structural contractum
  are equal, and cease to be under the substitution that makes the
  family constant --- whence \emph{no substitution-closed
  presentation of the algorithmic equality exists}
  (Corollary~\ref{cor:nopresentation});
...
\item a fundamental theorem in \emph{conditional} form
  (conversion instances restricted to environment-uniform ones,
  Theorem~\ref{thm:fundamental}), the failing step of the
  unrestricted argument exhibited exactly
  (Remark~\ref{rem:conversiongap}); and its repair on the core: an
  \emph{endpoint-blind hereditary-termination invariant} whose
  fundamental theorem tolerates unrestricted conversions, regaining
  canonicity, termination and decidability there
  (Theorem~\ref{thm:canonregained},
  Appendix~\ref{app:htrepair}).  The base-rule cases are inherited
  as an explicitly delimited \emph{base component}
...
\item an \emph{unconditional core}: for the minimal fragment
  ...
  (Appendix~\ref{app:core}); there the structural properties and the
  conditional fundamental theorem need no assumption, and the
  definitional $\J\,d\,\refl$ holds outright by a direct evaluation
  argument (Theorem~\ref{thm:core},
...
\item \emph{Unconditional theorem} (Theorem~\ref{thm:core},
  ...
  (Proposition~\ref{prop:J}) and canonicity, termination and
  decidability hold unrestrictedly
  (Theorem~\ref{thm:canonregained});
\item \emph{Relative theorem} (Theorems~\ref{thm:fundamental},
  \ref{thm:admissible}): the full implemented De~Morgan calculus,
  relative to the delimited base component (B) of
  Section~\ref{sec:assumptions}, with the fundamental theorem in
  conditional form;
...
\begin{thm}[Fundamental theorem, conditional form]
  \label{thm:fundamental}
  Call a conversion instance $\Gamma \vdash B \aconv A$
  \emph{environment-uniform} if $\evalop(B,\rho)$ and
  $\evalop(A,\rho)$ decode to the same PER for \emph{every} pair of
  related computable $\Gamma$-environments (not only the generic
  one).  If $\Gamma \vdash t : A$ in $T_{\mathrm{alg}}$ by a
  derivation all of whose conversion instances are
  environment-uniform, then $\Gamma \vDash t : A$.
  (By Remark~\ref{rem:conversiongap} the uniformity hypothesis is
  not vacuous: accepted conversion instances exist that are not
  environment-uniform, so the restriction cannot be dropped;
  conversion-free derivations satisfy it trivially.)
\end{thm}
\begin{proof}
...
\begin{cor}[Termination, under the same hypothesis]
...
\begin{thm}[Structural properties of the typed equality]
  ...
  \item is stable under weakening (Kripke monotonicity of $\valeq$
    in the depth);
  \item is stable under \emph{renamings} (substitutions sending
    variables to generic variables): a renaming maps the generic
    environment to a generic environment, so Lemma~\ref{lem:subst}
    transfers acceptance directly.  It is \emph{not} stable under
    general substitution: Theorem~\ref{thm:nonsubst} below gives a
    machine-checked counterexample, and
    Corollary~\ref{cor:nopresentation} shows the failure is
    intrinsic, not a defect of the checker;
  ...
  \item interfaces with typing in $T_{\mathrm{alg}}$: if
    $\Gamma \vdash t : A$ and $\Gamma \vdash A \aconv B$ then
    $\Gamma \vdash t : B$ (this \emph{is} $T_{\mathrm{alg}}$'s
    conversion rule), and the bidirectional checker's conversion
    sites decide exactly this relation.  Whether this rule is
    semantically harmless is precisely the question reopened by
    Theorem~\ref{thm:nonsubst}; see
    Remark~\ref{rem:conversiongap}.
  \end{enumerate}
\end{thm}
...
\section{The second no-go: non-substitutivity of the typed
  equality}
\label{sec:nonsubst}

The no-go theorem of Section~\ref{sec:nogo} rules out equational,
substitution-closed presentations.  The operational system evades it
by being algorithmic --- and this section shows the evasion is
\emph{exact}: the typed algorithmic equality itself fails
substitution-stability, one instance at a time, and provably must.

Work over $\Gamma = (A : \U,\ a\,b : A,\ p : \Path\,A\,a\,b,\
q : \Path\,A\,a\,a)$ and let
\begin{align*}
  t &\;:=\; \transp^i\,(\Path\,A\,a\,(p\,i))\ \io\ q,\\
  u &\;:=\; \langle j \rangle\, \hcomp^{\,\iota} A\,
    [\,j{=}0 \mapsto a,\ j{=}1 \mapsto p\,\iota\,]\,(q\,j)
\end{align*}
--- a transport redex over a family that is \emph{non-constant at the
generic environment}, and its structural contractum (the transport
components along the constant line $A$ collapse by (R) during
evaluation).  Let $\sigma := [\refl_a/p,\ a/b]$ into
$\Delta = (A, a, q)$.

\begin{thm}[Non-substitutivity]\label{thm:nonsubst}
  All three of the following hold in the algorithmic equality
  (machine-checked, \texttt{LibNonSubst.lean}):
  \begin{enumerate}
  \item $\Gamma \vdash t \aconv u$ --- the evaluator's negative
    branch \emph{is} the structural rule, so redex and contractum
    have the same value at the generic environment;
  \item $\Delta \vdash t\sigma \aconv q$ --- after $\sigma$ the
    family's \emph{value} is constant, so the check fires;
  \item $\Delta \vdash u\sigma \not\aconv q$ and
    $\Delta \vdash t\sigma \not\aconv u\sigma$ --- the substituted
    contractum freezes into a constant-tube $\hcomp$, the
    \eqref{eq:dagger}-residue, separated from $q$ by
    Proposition~\ref{prop:papersep}.
  \end{enumerate}
  Hence $\aconv$ is not closed under substitution: the equation
  $t \aconv u$ holds while its $\sigma$-instance fails.  The root
  cause is that \emph{normal forms are not substitution-stable} in
  the prioritized system: $\nf(t)$ is the composite form while
  $\nf(t\sigma) = q$.
\end{thm}

\begin{cor}[No presentation]\label{cor:nopresentation}
  No equality that coincides with the algorithmic equality is closed
  under substitution; in particular the algorithmic equality has no
  substitution-closed rule-generated presentation.  This resolves
  affirmatively the conjecture left open in earlier versions of this
  work.
\end{cor}
\begin{proof}
  A substitution-closed equality containing (1) and (2) contains
  $t\sigma \conv u\sigma$ and hence, by transitivity and symmetry,
  $u\sigma \conv q$ --- which (3) excludes from the algorithmic
  equality.
\end{proof}

\begin{rem}[The conversion gap]\label{rem:conversiongap}
  Theorem~\ref{thm:nonsubst} invalidates a step in the fundamental
  theorem as previously formulated: its conversion case inferred,
  from acceptance of $\Gamma \vdash B \aconv A$ \emph{at the generic
  environment}, the equality of the induced PERs at \emph{every}
  computable environment.  A type-level counterexample is now
  available: $B := \Path\,(\Path\,A\,a\,b)\,t\,t$ and
  $A' := \Path\,(\Path\,A\,a\,b)\,t\,u$ are accepted at the generic
  environment (their endpoint values coincide there,
  Theorem~\ref{thm:nonsubst}(1)), yet at the environment sending
  $p \mapsto \refl_a$ their endpoint values differ
  (Theorem~\ref{thm:nonsubst}(3)), so they decode to different PERs.
  The transfer implicitly used normal-form substitution-stability,
  which fails.  Consequently the fundamental theorem holds as stated
  only for derivations whose conversion instances are semantically
  valid at every environment
  (Theorem~\ref{thm:fundamental}), and canonicity is
  \emph{re-scoped as a conjecture}
  (Conjecture~\ref{thm:canonicity}); Section~\ref{sec:whatstands}
  delimits exactly what stands.
\end{rem}

\subsection{What stands, unaffected}
\label{sec:whatstands}

The following are independent of the fundamental theorem's
conversion case and are unaffected: the first no-go theorem
(Theorem~\ref{thm:nogo}) and the path-separation paper proof
(Proposition~\ref{prop:papersep}); soundness into $T_{\mathrm{eq}}$
(Theorem~\ref{thm:soundness} --- per-derivation, no conversion
transfer involved); checker exactness (Theorem~\ref{thm:exact}); the
evaluator-level substitution lemma (Lemma~\ref{lem:subst} --- it
relates the \emph{same} term under environment composition and is
consistent with Theorem~\ref{thm:nonsubst}, which concerns the
\emph{typed relation at generic environments}); the PER, congruence
and renaming clauses of Theorem~\ref{thm:admissible}; consistency of
the checker ($0 \not\aconv 1$ is a direct fact); the entire
strictness catalogue; and $\J\,d\,\refl \aconv d$, which is
re-proved by a direct evaluation argument in
Proposition~\ref{prop:J} without appeal to the fundamental theorem.
Moreover canonicity itself is \emph{regained} on the minimal core by
an invariant designed around this very phenomenon
(Theorem~\ref{thm:canonregained}).

\section{Canonicity}
\label{sec:canonicity}

\begin{conj}[Canonicity]\label{thm:canonicity}
  Every closed $T_{\mathrm{alg}}$-term $t : \mathbb{N}$ evaluates to
  a numeral.
\end{conj}

\noindent The status is precise.  For derivations whose conversion
instances are environment-uniform, canonicity is a theorem (the
argument below, via Theorem~\ref{thm:fundamental}).  For the
\emph{minimal core}, canonicity is regained \emph{unrestrictedly} by
a different invariant --- unary and endpoint-blind, hence immune to
the non-substitutivity phenomenon
(Theorem~\ref{thm:canonregained}; development in
Appendix~\ref{app:htrepair}).  For the full calculus with universes
the conjecture remains open: there, types are terms and the
skeleton-syntacticity on which the repair rests is lost.
Consistency of the algorithmic equality ($0 \not\aconv 1$) and
$\J\,d\,\refl \aconv d$ are not conditional in any case.

\begin{thm}[Canonicity regained for the minimal core]
  \label{thm:canonregained}
  Over the minimal core (no $\Glue$, no universes, no HITs), with
  \emph{no} restriction on conversion instances and \emph{no}
  base-component assumption: every closed
  $T^{\mathrm{core}}_{\mathrm{alg}}$-term $t : \mathbb{N}$ evaluates
  to a numeral, and evaluation, read-back and conversion terminate
  on core-typed inputs; consequently the algorithmic equality is
  decidable on the core.
\end{thm}
\begin{proof}[Proof idea; full development in
  Appendix~\ref{app:htrepair}]
  The relational fundamental theorem is irreparable
  (Theorem~\ref{thm:nonsubst}), but canonicity needs only
  \emph{termination}, which is unary.  Core types embed terms
  \emph{only at path endpoints}; define hereditary termination
  $\mathrm{HT}_{T,\rho}$ by structural recursion on the syntactic
  type, with the $\mathsf{PathP}$-clause \emph{ignoring the
  endpoints}.  Then $\mathrm{HT}$ depends only on the type's
  \emph{skeleton}, which (i) is preserved by interval substitution
  --- so the positive transport branch closes trivially --- and
  (ii) is determined by the generic read-back --- so accepted
  conversions preserve $\mathrm{HT}$ \emph{at every environment},
  which is exactly where the binary PER argument failed.  The
  fundamental theorem for $\mathrm{HT}$ then goes through with an
  unrestricted conversion case, and closed $\mathrm{HT}$-values at
  $\mathbb{N}$ are numerals.
\end{proof}
\begin{proof}[Proof of the environment-uniform case]
...
\begin{cor}[Minimal core, without base assumptions]\label{thm:core}
  Over the minimal fragment $\Pi, \Sigma, \Path, \mathbb{N}$ with
  $\transp$ and $\hcomp$ (no $\Glue$, no universes, no HITs),
  Appendix~\ref{app:core} constructs the logical relation by
  structural recursion on syntactic types and proves every
  (B)-marked lemma for it, former by former.  Consequently the
  structural properties of Theorem~\ref{thm:admissible}, the
  conditional fundamental theorem
  (Theorem~\ref{thm:fundamental}) and hence canonicity for
  derivations with environment-uniform conversions hold there
  \emph{with no base-component assumption}.  The scope caveat of
  Remark~\ref{rem:conversiongap} applies inside the core as well ---
  the non-substitutivity counterexample lives in the core --- but
  there it is \emph{overcome}: unrestricted core canonicity,
  termination and decidability hold by the endpoint-blind invariant
  (Theorem~\ref{thm:canonregained},
  Appendix~\ref{app:htrepair}), and the headline equation
  $\J\,d\,\refl \aconv d$ holds by the direct evaluation argument of
...
(\texttt{LibSwitchover.lean}), the non-substitutivity probes
(\texttt{LibNonSubst.lean}), library, harness, and a row-by-row
paper-to-probe correspondence (\texttt{ARTIFACT.md}) --- is public at
\begin{center}
  \url{https://github.com/r-shibusawa/cubical-strict-j}
\end{center}
release \texttt{v1.1.0} (this revision).  The pre-erratum snapshot
\texttt{v1.0.0} is archived at DOI
\href{https://doi.org/10.5281/zenodo.21405962}%
{\texttt{10.5281/zenodo.21405962}}; \texttt{v1.1.0} is archived as a
new version under the same Zenodo record.  Pinned toolchain
...
already (Theorem~\ref{thm:nogo}) --- and, in the second no-go
theorem, that the operational evasion is exact: the typed
algorithmic equality is a congruent, renaming-stable partial
equivalence that is provably \emph{not} substitution-stable
(Theorem~\ref{thm:nonsubst}) and provably has no
substitution-closed presentation
(Corollary~\ref{cor:nopresentation}).  The system still computes
$\J$ on $\refl$ --- unconditionally, by a direct evaluation
argument on the minimal core (Proposition~\ref{prop:J}) --- and is
sound for $T_{\mathrm{eq}}$ with an exactly-specified checker; and
on the minimal core, canonicity, termination and decidability are
\emph{regained}, with unrestricted conversions, by the
endpoint-blind termination invariant
(Theorem~\ref{thm:canonregained}) --- the unary layer of the
metatheory crosses the substitution barrier that provably kills the
binary layer.  We regard the pair of no-go theorems, the
delimitation of what survives them, and this unary/binary
separation as the paper's contribution; no unconditional strict
$\J$ for the full univalent system is claimed.
...
equality.
(4)~\emph{Canonicity beyond the core}: extending the
endpoint-blind invariant (Appendix~\ref{app:htrepair}) past
universes, where types are terms and neutral codes have no
syntactic skeleton; the core case is settled by
Theorem~\ref{thm:canonregained}, and the presentation question
previously listed here by Corollary~\ref{cor:nopresentation}.
...
Theorem~\ref{thm:fundamental} (in its conditional form) and
Theorem~\ref{thm:admissible} without base assumptions
...
\begin{proof}
  ...
  every step is a concrete, finite, machine-reproducible
  computation on this instance --- family evaluation, occurs-check,
  the positive branch, and one conversion of identical values ---
  requiring neither the fundamental theorem nor any base-component
  assumption.
...
\subsection{The endpoint-blind invariant: canonicity regained}
\label{app:htrepair}

This subsection proves Theorem~\ref{thm:canonregained}: unrestricted
core canonicity, immune to Remark~\ref{rem:conversiongap}.

\subparagraph*{Skeletons.}  Core types embed terms only as
$\mathsf{PathP}$ endpoints ($\Pi$ and $\Sigma$ carry types; $\mathbb{N}$
nothing).  The \emph{skeleton} $\mathrm{sk}(T)$ erases the endpoint
terms.  Interval substitution changes no former, so
$\mathrm{sk}(T(\io/i)) = \mathrm{sk}(T(\ii/i)) = \mathrm{sk}(T)$.
The non-substitutivity counterexamples perturb only endpoint
\emph{values} across environments; skeletons are environment-blind.

\subparagraph*{Hereditary termination.}  For a syntactic core type
$T$ and environment $\rho$ (at a world $w$), a \emph{unary},
Kleene-read predicate on values:
\begin{align*}
  \mathrm{HT}_{\mathbb{N},\rho}(v) &\iff
    v \text{ forces to a numeral, or to a neutral/stuck value with }
    \mathrm{HT} \text{ components};\\
  \mathrm{HT}_{\Pi x:T.S,\rho}(f) &\iff
    \forall w' \to w,\ \forall u \in \mathrm{HT}_{T,\rho}\colon\\
    &\qquad
    f\,u \text{ defined and } \in \mathrm{HT}_{S,\rho+[u]};\\
  \mathrm{HT}_{\Sigma x:T.S,\rho}(v) &\iff
    \fst v \in \mathrm{HT}_{T,\rho} \text{ and }
    \snd v \in \mathrm{HT}_{S,\rho+[\fst v]};
\end{align*}
and, for $P = \mathsf{PathP}(\langle i \rangle T)\,t\,u$,
\[
  \mathrm{HT}_{P,\rho}(p) \iff
  \forall \iota\colon\ p\,\iota \text{ defined and }
  \in \mathrm{HT}_{T,\rho+[\iota]}
  \qquad(\text{\emph{endpoints $t,u$ ignored}}).
\]
Environments are $\mathrm{HT}$ pointwise; neutrals with
$\mathrm{HT}$ components are $\mathrm{HT}$ at every skeleton (the
unary reflection lemma, by structural induction).  By construction
$\mathrm{HT}$ depends only on $\mathrm{sk}(T)$ and the sub-syntax;
in particular
$\mathrm{HT}_{T(\io/i),\rho} = \mathrm{HT}_{T(\ii/i),\rho}$
\emph{literally}.

\begin{lem}[Conversion invariance]\label{lem:htconv}
  If $T \sim_{\mathrm{gen}} T'$ (equal generic read-backs, i.e.\
  accepted conversion, Lemma~\ref{lem:convrb}), then
  $\mathrm{HT}_{T,\rho} = \mathrm{HT}_{T',\rho}$ for \emph{every}
  $\rho$.
\end{lem}
\begin{proof}
  Induction on the skeleton; core type values are always canonical,
  so equal read-backs force equal head formers.  $\Pi$/$\Sigma$:
  read-back opens the codomain generically, giving
  $T_1 \sim_{\mathrm{gen}} T_1'$ and (in the extended context)
  $S \sim_{\mathrm{gen}} S'$; the induction hypotheses equate the
  argument and result predicates at all environments, and the
  clauses are built from nothing else.  $\mathsf{PathP}$: the lines
  are $\sim_{\mathrm{gen}}$ under the interval binder (induction
  hypothesis); \emph{the endpoint read-backs, though equal, are not
  needed} --- the clause ignores endpoints.  This is precisely the
  step at which the binary PER argument failed
  (Remark~\ref{rem:conversiongap}) and the unary endpoint-blind
  predicate does not.
\end{proof}

\begin{thm}[Fundamental theorem for $\mathrm{HT}$]\label{thm:htfund}
  If $\Gamma \vdash t : A$ in $T^{\mathrm{core}}_{\mathrm{alg}}$
  (conversion \emph{unrestricted}) and $\rho$ is an
  $\mathrm{HT}$-environment, then $\evalop(t,\rho)$ is defined and
  $\mathrm{HT}_{A,\rho}$.
\end{thm}
\begin{proof}
  Induction on the typing derivation, with the inner structural
  induction on syntactic types of Remark~\ref{rem:order}; the cases
  are the unary shadows of
  Sections~\ref{app:corerel}--\ref{app:coretransp}.
  \emph{Conversion:} Lemma~\ref{lem:htconv}, with no uniformity
  hypothesis.  \emph{Transport, positive branch:} the result is
  $\evalop(u,\rho)$ with $u : A(\io/i)$ against result type
  $A(\ii/i)$; the two $\mathrm{HT}$ predicates are literally equal
  (skeleton invariance under interval substitution) --- the
  checker-soundness theorem is not even needed here.
  \emph{Transport, negative branch, and $\hcomp$:} the structural
  dispatches recurse along proper sub-skeletons (domain lines,
  codomain fibers over fills, endpoint-extended systems), exactly
  as in Lemmas~\ref{lem:corehcompN}--\ref{lem:coretransp} with the
  relational content dropped; definedness is carried by every
  clause.  \emph{$\mathsf{natrec}$:} numeral scrutinees by an inner
  numeral induction; neutral/stuck scrutinees by the unary
  reflection lemma.
\end{proof}

\begin{proof}[Proof of Theorem~\ref{thm:canonregained}]
  The closed instance of Theorem~\ref{thm:htfund}: the empty
  environment is $\mathrm{HT}$.  A closed $\mathrm{HT}$-value at
  $\mathbb{N}$ is a numeral: closed neutrals need a variable, and
  closed stuck composites need an undecided face, which closed
  cofibrations do not have (the closed-instance remark of
  Lemma~\ref{lem:corehcompN}).  Termination of read-back and
  conversion on core-typed inputs follows from $\mathrm{HT}$ of the
  compared values along the recursions of
  Sections~\ref{app:corereify}--\ref{app:coreconv}; decidability of
  the algorithmic equality on the core is then immediate.
\end{proof}

\begin{rem}[The boundary of the repair]
  The repair is deliberately \emph{unary}: the binary (relational)
  fundamental theorem stays dead, as it must
  (Theorem~\ref{thm:nonsubst}).  And it is deliberately
  \emph{core-bound}: with universes, types are terms --- neutral
  type codes have no syntactic skeleton --- and whether an
  environment-invariant ``skeleton of a code value'' can be carved
  out is the open problem to which
  Section~\ref{sec:conclusion}(4) now points.  The general design
  principle this exhibits: in a system whose judgmental equality
  incorporates evaluation-time optimization, the metatheory
  separates into a unary layer (termination, canonicity) that
  crosses the substitution barrier and a binary layer (semantic
  equality) that provably cannot.
\end{rem}
