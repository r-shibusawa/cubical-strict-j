...
\begin{abstract}
  ...
  Eilenberg--Zilber structure.  All counts and theorems are verified
  exhaustively by machine through three generators.
\end{abstract}
...
step.  At $n = 2$ the boundary counts of the Kleene and De Morgan
...
The subvariety classification suggests two further questions.
First, \emph{relative theories}: interval theories between Kleene
and De Morgan can be manufactured by bounding the number of
``independently mixable'' variable groups; the fiber theory should
interpolate in a graded way between two poles and the full diagonal
antichain lattice, and it would be interesting to axiomatize which
fiber behaviours are realizable by \emph{some} interval theory.
Second, \emph{the strict--weak interface}: in the De Morgan case
