...
Twenty-two self-contained papers have been developed on this artifact; each is independently readable, with all machine-checked claims anchored in this repository.
...
| 18 | [A one-line separation criterion for isotropy quotients of cubical sites with reversal](docs/consolidated/criterion.pdf) | the case-by-case classification collapses to a single criterion, uniform in the dimension and the site: an isotropy quotient agrees iff the normal closure of the reflections of its symmetry group still fixes a cell, and carries an attached separating map iff that closure is itself mixed — proved by an exact characterization of the certificate method (certified = reflection-generated) and a two-step covering argument that subsumes the median and product families; reproduces the 78/20 table with zero mismatches |
| 19 | [Reflection subgroups and the homotopy theories of cubical sites](docs/consolidated/reflection.pdf) | the integrated account of papers 16–18, extended to a main theorem at **every** dimension: on the Boolean site the strict homotopy component of the identity of an isotropy quotient consists exactly of twisted-equivariant vertex maps (sieve-landing dichotomy), so a mixed reflection closure either separates outright or reduces, in both structures at once, to a strictly smaller codiscrete quotient — a well-founded recursion (reflection transport) making SEP(H) ⟺ ⟨⟨Refl(H)⟩⟩ mixed a theorem for all n; at n = 4 a closed-form census decides all 30 classes (28 rigid, 2 reductions — one landing on the Klein quotient itself); with a fundamental-group formula for isotropy quotients, a collage formula reducing every el to finite group cohomology plus poset data, and a bridge theorem: the orbit space of the Milnor construction of a finite M-set is F₂-acyclic whenever a Sylow 2-subgroup has a fixed point |
| 20 | [The separation criterion at every dimension on every cubical site with reversal](docs/consolidated/allsites.pdf) | the all-dimension criterion extends from the Boolean site to De Morgan and Kleene — i.e. to every standard cubical site with reversal: the De Morgan convexity interpolant is self-dual, so shared-deck cells are one-step homotopic on every reversal site and the sieve-landing dichotomy transfers verbatim; theory quotients project rigidity from the Boolean site, and stabilized cells reduce algebraically (no power trick) to lower-dimensional isotropy quotients; the n = 4 reduction classes admit De Morgan/Kleene witnesses on the same invariant plane as their Boolean ones (exact 2-SAT) |
| 21 | [Dedekind cubical sets do not present spaces](docs/consolidated/dedekind.pdf) | negative resolution of the last open comparison among the standard cubical-type model structures: the type structure on Dedekind cubical sets (two connections) is NOT equivalent to spaces — witnessed by the **monotone dunce hat**, a two-relation quotient of the square that is test-contractible yet type-rigid; the proof is a provenance analysis of open-box fillings whose heart is a one-line monotonicity lemma (a monotone map constant on the diagonal cylinder is constant); with Cavallo–Sattler's one-connection theorem this completes the phase diagram: exactly one standard cubical site presents spaces — **RETRACTED** (v1.22.0): the separation claim is false; the monotone dunce hat IS type-contractible, by a three-line *shell* filling available in every uniformly fibrant object (the cellular open-box analysis of the paper is correct but does not exhaust uniform fibrancy); see paper 22, Theorem C — the Dedekind entry is re-opened |
| 22 | [Which cubical sites present spaces?](docs/consolidated/cubicalsites.pdf) | the integrated phase-diagram manuscript (98 pages; subsumes papers 13–21 and corrects paper 21): the reflection criterion settles every reversal site at every dimension; the monotone dunce hat is NO witness (shell contraction — Theorem C); the Dedekind comparison is transformed into a chain-approximation problem and driven deep into it — the singular complex of the order-statistics triangulation computes the test homotopy type of every presheaf, chain approximation is proved for all chain-cell quotients, all cyclic atoms and over the square, the cone-descent theorem contracts every split-pair fold quotient, and an adversarially audited reduction leaves exactly two open combinatorial statements (subatom well-foundedness; intrinsic-chain-anchored asphericity); archived at DOI [10.5281/zenodo.22247384](https://doi.org/10.5281/zenodo.22247384) |

Papers 2–12 cite paper 1 as the platform; papers 13–15 build on the consolidated test-comparison manuscript below (paper 15 is self-contained up to the standard collage formula); paper 19 integrates and supersedes papers 16–18; papers 20–21 completed the criterion at every dimension (paper 21's phase-diagram claim is retracted and corrected by paper 22, which subsumes 13–21); release-by-release DOIs are listed in [`ARTIFACT.md`](ARTIFACT.md).
